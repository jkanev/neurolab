...
int plot(Time *time, double n, double w, double lambda, double n_rc, double lambda_rc, double, double, double, double, string);
...
{ 
	...
	double dt = 0.1;
	double w = 0.5;
	double lambda = 0.1*Hz;
	double n_rc = 0.1;
	double lambda_rc = 0.2;
	double n = 500;
	double v0 = -60;
	double theta = -54;
	double tau = 5.0;
...
//	WienerCpp cppBg( dt, w, n, lambda, 0.0, 0.0 ); // reference cpp with no coincidences
	...
	// calibrate
	nrn.calibrate(isi, 1000, 1000);
	
	// measure isi-density
	IntervalEstimator estRsc( dt, EST_DIFF | EST_MEAN | EST_VAR, &nrn );
	for( int i=0; i<5000; i++ ) {
		nrn.proceedToNextState();
		nrn.prepareNextState();
		estRsc.proceedToNextState();
	}
	
	cout << "desired rate: " << isi << ", measured rate: " << estRsc.getEstimate(EST_MEAN) << endl;
