...
*/
class ConditionalEstimator: public Estimator, public TimeDependent
{
private:
	Event *pTrigger; ///< event triggering record action
	Stochastic *pCondition; ///< condition
	unsigned int nCount; ///< countdown after an event has occured
	Ring<double> rSample; // current sample
	Ring<bool> rEvents; ///< ring with circling events
	Ring<double> rConditions; ///< ring with circling conditions
	double *aSample; // recording sample
...
public:
	...
	ConditionalEstimator(
		...
		Stochastic *stochvar,  ///< the address of the stochastic variable to record
		Event *condition, ///< the address of the event on which the recording is conditioned
	...
	ConditionalEstimator(
		const Property& property, ///< a collection of things to record
		Stochastic *stochvar,  ///< the address of the stochastic variable to record
		Stochastic *condition, ///< the address of the process on which the recording is conditioned
		int pre, ///< number of time steps before the event 
		int post, ///< number of time steps after the event 
		int updateCorrection=0 ///< optional; defaults to 0. number of time steps to shift the result;  this is to correct different update cascades
	);
