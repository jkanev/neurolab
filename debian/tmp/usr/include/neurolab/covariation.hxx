...
class Covariation: public Stochastic
{
private:
	Stochastic *xFirst;
	Stochastic *xSecond;
...
public:
	...
	Covariation(Stochastic *x, Stochastic *y) : Stochastic(x->getTime()) {
	...
	/*! If the process is active, this is called automatically. If it is passive, call step() first, and then next(), to complete a full step.. */
	void next() {
		dNextValue += xFirst->dDelta() * xSecond->dDelta();
	};
};
